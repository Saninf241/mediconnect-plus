...
export const handler: Handler = async (event) => {
  try {
    ...
    const {
      ...
      operator_id,
      template_b64,
      template_hash,     // optionnel désormais
      quality,
      device_model,
    } = payload;
    ...
    // Si le hash n’est pas fourni, on le calcule (SHA-256 b64)
    const hash =
      template_hash ||
      createHash("sha256").update(Buffer.from(template_b64, "base64")).digest("base64");
    ...
    const { error } = await supabase
      ...
      .upsert(
        ...
        { onConflict: "patient_id" } // ← correspond à ton index unique sur patient_id
      );
