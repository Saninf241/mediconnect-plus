...
export const handler: Handler = async (event) => {
  try {
    ...
    const scope = event.queryStringParameters?.scope || "clinic"; // "clinic" | "global"
    ...
    if (scope === "clinic") {
      if (!clinic_id) return { statusCode: 400, body: "clinic_id required" };
      query = query.eq("clinic_id", clinic_id);
    } else {
      // global: pas de filtre (interop inter-cliniques)
    }
